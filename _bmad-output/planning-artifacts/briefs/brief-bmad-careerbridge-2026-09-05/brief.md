...
Business rules that keep it honest: recruiter and organization accounts need administrator approval; postings need approval and expire; an applicant may withdraw but not edit a submitted application and may hold at most five active applications; a recruiter sees only applications to their own organization's jobs.
...
| **Recruiter / HR** | Person hiring for a small organization, possibly for more than one | Posts once, reviews applicants in one queue, records every decision without losing anyone |
| **Administrator** | Platform operator | Controls who may recruit, what goes live, and the reference data; nothing reaches the public unreviewed |
...
2. The approval gates, the posting expiry, and the five-active-applications cap are enforced, not decorative.
3. Any requirement can be followed from its ID to a use case, to the code, and to the test that covers it.
...
- **Evidence.** Individual contribution is visible in Git history and confirmed by peer evaluation at the end of each iteration.
- **Stack rationale.** JavaScript end to end (Node.js backend, React frontend, deliberately not TypeScript), approved verbally by Dr. Ren; the written rationale for departing from Maven and JUnit is due in Iteration 1.

## Scope

**In scope for the semester:** the three roles above, the fixed pipeline, public job browsing, applicant self-registration, admin approval of recruiter accounts and postings, one resume per applicant, withdraw-not-edit, the five-active cap, all stages visible to the applicant, recorded interviews and offers, rejection reasons, auto-close on fill, per-organization privacy, in-app notifications, a relational data model, automated tests, web deployment.

**Explicitly out:** interview scheduling and calendar coordination (only the fact and outcome are recorded); multiple resumes or cover letters per application; configurable pipelines; email notifications; any AI feature (resume parsing, matching, feedback) unless the core scope is complete and the team chooses a stretch. Anything cut is cut to protect the fifteen use cases and the demo.

### First-cut use-case split (for team discussion)
...
| --- | --- | --- | --- | --- |
| UC-A1 | Applicant | Register and maintain applicant profile, including single resume upload | M | 1 |
| UC-A2 | Applicant | Browse and search open postings (public and logged in) | M | 1 |
| UC-A3 | Applicant | Apply to a posting (enforces one resume, no duplicate, five-active cap) | M | 2 |
...
| UC-R1 | Recruiter | Register an organization and recruiter account (pending approval); maintain org profile; join a second org | M | 1 |
...
| UC-M3 | Admin | Manage user accounts and roles (suspend, reactivate, reassign) | M | 2 |
| UC-M4 | Admin | Maintain reference data (job categories, locations, pipeline stage labels) and an oversight view | M | 3 |

Count by role: applicant 5, recruiter 6, admin 4. The admin column is lighter on purpose because the admin model is open (D-005):

- **Option A, single platform admin.** UC-M1 to UC-M4 as listed. Admin stays at four; a fifth admin use case could be an audit log of approvals and decisions.
- **Option B, platform admin plus company admin.** Add UC-M5 "Company admin manages their organization's recruiters and hiring managers", moving recruiter-management out of UC-R1 and making the split 5 / 5 / 5. This also gives the privacy rule a natural owner.

Suggested ownership rule: each member owns three use cases from at least two different roles, and every member has at least one use case on the golden path, so nobody is a bystander at the demo.

## Open Questions

- Admin model, Option A or B (D-005). To settle with Dr. Ren in Iteration 1.
- The five-active-applications cap sits oddly next to an applicant who applies to twenty places. Confirm the number with the customer or reframe it as a configurable business rule.
...
- Data retention when an account is deleted; where the course AI-use policy lives; whether Dr. Ren remains the customer all semester.
